console.log('Batch extraction - FINAL VERSION with proper structure');
...
// Step 2: Extract instruments from the Contents section
function extractInstrumentChecklist() {
  ...
  // Look for the table with "Item ID" header (the contents table)
  ...
  if (!contentsTable) {
    console.log('      ⚠ Could not find contents table with Item ID column');
    return [];
  }

  // Extract rows from the contents table
  let rows = contentsTable.querySelectorAll('tbody tr');

  rows.forEach(row => {
    let cells = row.querySelectorAll('td');

    // Table structure: Item ID | Item | Type | Quantity
    if (cells.length >= 4) {
      ...
      // Only add if itemId matches expected format (letters/numbers, not empty)
      // Valid formats: XE1462, S0090285, XS1583, 500539, XX1922, etc.

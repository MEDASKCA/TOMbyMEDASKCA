...
export default function StaffCompetencyModal({
  ...
  const [selectedSpecialtyForDetail, setSelectedSpecialtyForDetail] =
    useState<any>(null);
    ...
    return {
      ...
      canRelieveIn: [
        { theatre: "Theatre 1", available: true, reason: "Primary theatre" },
        {
          theatre: "Theatre 2",
          available: true,
          reason: "Competent in General",
        },
        {
          theatre: "Theatre 3",
          available: false,
          reason: "Not certified for Cardiac",
        },
        {
          theatre: "Theatre 4",
          available: false,
          reason: "Neuro - Learning only",
        },
        {
          theatre: "Theatre 5",
          available: true,
          reason: "Emergency certified",
        },
      ],
      todaysActivity: {
        casesCompleted: 3,
  ...
  return (
    <div className="fixed inset-0 bg-gray-100 bg-opacity-95 flex items-center justify-center z-50">
      <div className="bg-white shadow-xl w-full h-full overflow-hidden flex">
        {/* Left Staff Info Panel - Desktop Only */}
        <div className="hidden lg:flex lg:flex-col lg:w-80 bg-gray-50 border-r border-gray-200 overflow-y-auto p-4 flex-shrink-0">
          {/* Staff Header */}
          <div className="border-b border-gray-200 pb-3 mb-3">
            <h3 className="font-bold text-gray-900 text-lg">{staff.name}</h3>
            <p className="text-sm text-gray-600">
              {staff.role} • {staffDetailsForContext.grade}
            </p>
            <p className="text-xs text-gray-500 mt-1">
              ID: {staffDetailsForContext.employeeId}
            </p>
            <div className="mt-2 pt-2 border-t border-gray-200">
              <p className="text-xs text-gray-500">Shift</p>
              <p className="text-sm font-medium">
                {staffDetailsForContext.shiftStart} -{" "}
                {staffDetailsForContext.shiftEnd}
              </p>
            </div>
          </div>
            ...
            <div className="space-y-2 text-xs">
              <div>
                ...
                <p className="font-medium text-gray-900">
                  {staffDetailsForContext.currentLocation}
                </p>
              </div>
              <div>
                <span className="text-gray-500">Break Status:</span>
                <p
                  className={`font-medium ${staffDetailsForContext.breakStatus.taken ? "text-green-600" : "text-orange-600"}`}
                >
                  {staffDetailsForContext.breakStatus.totalBreaks} taken
                </p>
                {!staffDetailsForContext.breakStatus.taken && (
                  <div className="mt-1 px-2 py-1 bg-yellow-100 text-yellow-700 rounded text-xs">
                    Next break due: {staffDetailsForContext.breakStatus.nextDue}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="space-y-2">
              {specialties
                ...
                .map((comp) => (
                  <div
                    key={comp.id}
                    className={`flex items-center justify-between px-2 py-1.5 rounded border text-xs ${getCompetencyColor(comp.level)}`}
                  >
          ...
          <div className="mb-3">
            ...
            <div className="space-y-1">
              {staffDetailsForContext.canRelieveIn.map(
                (theatre: any, idx: number) => (
                  <div
                    key={idx}
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <p
                        className={`font-medium ${theatre.available ? "text-green-700" : "text-gray-500"}`}
                      >
                        {theatre.theatre}
                      </p>
                      <p className="text-xs text-gray-500">{theatre.reason}</p>
                    </div>
                  </div>
                ),
              )}
            ...
            <div className="space-y-1 text-xs">
              <div className="flex justify-between">
                ...
                <span className="font-medium text-gray-900">
                  {staffDetailsForContext.todaysActivity.casesCompleted}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Relief Provided:</span>
                <span className="font-medium text-gray-900">
                  {staffDetailsForContext.todaysActivity.reliefProvided}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Efficiency:</span>
                <span className="font-medium text-green-600">
                  {staffDetailsForContext.todaysActivity.averageEfficiency}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Overtime:</span>
                <span className="font-medium text-gray-900">
                  {staffDetailsForContext.todaysActivity.overtime}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          {viewMode !== "specialties" && (
            <div className="bg-gray-50 px-6 py-3 border-b border-gray-200">
              ...
              >
                ...
                <span>
                  Back to{" "}
                  {viewMode === "systems" ? "Procedures" : "Specialties"}
                </span>
              </button>
            </div>
          )}
          ...
          <div className="flex flex-1 overflow-hidden">
            ...
            <div
              className={`lg:hidden w-2/5 bg-gray-50 border-r border-gray-200 overflow-y-auto p-3 ${showSpecialtyDetail ? "hidden" : "block"}`}
            >
              {/* Staff Header */}
              <div className="border-b border-gray-200 pb-3 mb-3">
            ...
            {/* Right Content Area */}
            <div
              className={`flex-1 overflow-y-auto p-6 ${showSpecialtyDetail ? "w-full" : ""}`}
            >
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <div className="flex items-center space-x-3 mb-2">
                                ...
                                <span
                                  className={`px-2 py-1 rounded-full text-xs font-medium border ${getLevelColor(specialty.level)}`}
                                >
                                  {specialty.level}
                                </span>
                                {specialty.certified && (
                  ...
                  <div className="mt-6 p-4 bg-indigo-50 rounded-lg">
                    ...
                    <div className="grid grid-cols-3 gap-4 text-sm">
                      <div>
                        ...
                        <span className="ml-2">
                          {
                            specialties.filter((s) => s.level === "Expert")
                              .length
                          }{" "}
                          specialties
                        </span>
                      ...
                      <div>
                        ...
                        <span className="ml-2">
                          {specialties.filter((s) => s.certified).length}{" "}
                          specialties
                        </span>
                      </div>
                      <div>
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <div className="flex items-center space-x-3 mb-2">
                                ...
                                <span
                                  className={`px-2 py-1 rounded text-xs font-medium ${getComplexityColor(procedure.complexity)}`}
                                >
                ...
                <>
                        ...
                        >
                          <div className="flex items-start justify-between mb-3">
                            ...
                            <div className="text-right">
                              ...
                              <p
                                className={`text-sm font-medium mt-1 ${getRatingColor(system.rating)}`}
                              >
                          ...
                          <div className="grid grid-cols-3 gap-4 pt-3 border-t border-gray-200">
                            <div className="text-sm">
                              ...
                              <span className="ml-2 font-medium">
                                {system.casesCompleted}
                              </span>
                            </div>
                            <div className="text-sm">
                              <span className="text-gray-500">Last Used:</span>
                              <span className="ml-2 font-medium">
                                {system.lastUsed === "Never"
                                  ? "Never"
                                  : new Date(
                                      system.lastUsed,
                                    ).toLocaleDateString()}
                  ...
                  <div className="mt-6 p-4 bg-yellow-50 rounded-lg">
                    ...
                    <ul className="text-sm text-yellow-800 space-y-1">
                      {systems[selectedProcedure.id]
                        ?.filter((s) => s.rating <= 2)
                        .map((system) => (
                          <li key={system.id}>
                            • Schedule training for {system.manufacturer}{" "}
                            {system.name}
                          </li>
                        ))}
                      {systems[selectedProcedure.id]
                        ...
                        .map((system) => (
                          <li key={system.id}>
                            • Practice sessions recommended for{" "}
          ...
          <div className="bg-gray-50 px-6 py-3 border-t border-gray-200">
            <div className="flex items-center justify-between text-xs text-gray-500">
              <div className="flex items-center space-x-1">
                ...
                <span>
                  Competency data verified by Education & Training Department
                </span>
              </div>
